-- GROUPING
-- GROUP BY 철을 이용해서 결과 집합을 특정 열의 값에 따라 그룹화하는데 사용
...
select
    ...
    count(*) -- 매개변수와 위치에 따라 여러 사용방법이 있음
             -- 전체 행 중 일치하는 행이 몇 개인지 갯수를 변환하는 역할
from
    tbl_menu
group by
    category_code;
...
/*
 HAVING
 - GROUP BY와 함께 사용하며, 그룹화된 결과에 조건을 적용할 때 사용된다.
 - WHERE는 개별 해엥 조건을 적용하는 것과 달리
 - HAVING 절은 그룹화된 데이터에 조건을 적용한다.
 */
 -- menu_price,category_code로 그루핑된 데이터에서 category_code가 5 이상, 8 이하
select
    menu_price,
    category_code
from
...
-- 카테고리 코드 그룹별 평균 가격을 구하고, 평균 가격이 8000원 이상인 경우만 조회
select
    category_code,
    avg(menu_price)
from
...
-- 컬럼 두 개를 활용한 with rollup
-- 메뉴 가격별 통합 및 해당 메뉴 자격별 같은 카테고리의 총합
select
    menu_price,
    category_code,
    count(*),
...
having
    count(menu_code) >= 3;
-- 3. 특정 카페고리에서 주문 가능한 메뉴의 평균 가격이 10000원 이상인 경우
select
    category_code,
    count(menu_code)
from
