...
-- concat_ws(구분자, 문자열, 문자열 2...) : 문자열을 구분자랑 같이 이어붙임
...
-- LOCATE(부분 문자열, 기준 문자열) : INSTR랑 숫자만 반대
...
select format(123123123123213.45678, 3);
...
-- RTRIM(문자열) : 오른쪽 공백 제거
-- TRIM(문자열) : 양쪽 공백을 제거
-- TRIM(BOTH(양쪽), LEADING(왼쪽), TRAILING(오른쪽)) 있으면 해당 방향에 지정한 문자열을 제거할 수 있다.
