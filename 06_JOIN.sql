-- JOIN
-- 여러 개의 분산된 표를 조합해서 하나의 표로 만드는  기능
-- 관계형 데이터베이스에서 가장 중요한 파트이다.

-- 두 개 이상 테이블을 관련 있는 컬럼을 통해 결합하는 데 사용된다.
-- 두 개 이상 테이블은 반드시 연관있는 컬럼이 존재해야 하며, 이를 통해 JOIN된 테이블들의 컬럼을 모두 확인할 수 있다.
...
- 컬럼명이 같ㄱ서나, 다를 경우 ON으로 서로 연관있는 컬럽에 대한 조건을 작성하여 JOIN 한다.
*/
...
-- INNER JOIN은 교집합을 반환하기 때문에 JOIN 순서를 바꾸어도 영향이 없다.
select
    a.menu_name,
    b.category_name,
...
-- USING
-- 컬럼명이 같을 경우 USING으로 서로 연관있는 컬럼에 대한 조건을 작성하여 JOIN할 수 있다.
...
/*
LEFT JOIN
- 첫번째 (왼쪽) 테이블을 모두 조회하고, 왼쪽과 일치하는 오른쪽 테이블을 JOIN 해준다.
*/
select
    a.menu_name,
    b.category_name
...
/*
CROSS JOIN
- 두 테이블의 모든 가능한 조합을 반환하는 JOIN(모든 경우의 수를 다 구함) 카르테시안 곱이라고도 함.
  */
