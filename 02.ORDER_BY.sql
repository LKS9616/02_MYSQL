-- ORDER BY
-- select 문과 함께 사용하며 결과 집합을 특정 열이나 값에 따라 정렬하는 데 사용된다.
...
-- 컬럼을 연산해서 연산결과와 함께 정렬을 할 수 있다. order by 절을 이용해서 연산결과와 함께 정렬
select
    menu_code,
    menu_price,
...
-- 첫 번째 인자가 두 번째 인자 이후의 값들과 비교해서 일치하면 해당 위치의 값을 보여준다.
*/
